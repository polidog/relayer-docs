...
- **ドキュメント本文にバージョン注記を書かない**（`> **vX.Y.Z で追加。**`
  callout・`- **vX.Y.Z** から、…` bullet・見出しや表の `（vX.Y.Z）`・
  「以前は／破壊的変更」の旧バージョン対比、すべて不可）。機能は常に
  **現在形**で「ある」ものとして書く。バージョンは `--note` と PR 本文に
  だけ残す（2026-05 ユーザー決定: まだ過去バージョンを使わないので
  本文の差分表記はノイズ。背景は memory [[relayer-release-doc-update-flow]]）。
...
- **バージョン注記なし・現在形**で書く。「vX.Y.Z で追加」「以前は…」
  ではなく、その機能が最初から存在するかのように事実だけ書く。新機能の
  追加も、破壊的変更後の挙動も、現在の契約を述べるだけ（旧挙動との対比は
  書かない＝過去バージョンは使われない方針）。
...
## ドキュメント
/docs/<slug> に「…」を追記済み（本文はバージョン注記なし・現在形）。
本文は Turso が唯一の正でこのリポジトリに markdown は無いため、反映は
この PR の差分外。
## 差分
...
- [ ] 正しい doc スラッグに、**バージョン注記なし・現在形**で追記
      （本文・見出し・表・frontmatter description にバージョンを書いていない）
